...
export class CameraControls extends EventDispatcher {
   ...
   * @category Methods
   */
  dollyOnScroll(event: any, scrollDivisor = 10) {
    ...
    return this._dollyInternal(
      Math.log(Math.abs(event.deltaY / scrollDivisor)) *
  ...
  getTarget(out: _THREE.Vector3): _THREE.Vector3 {
    const _out =
      !!out && out.isVector3 ? out : (new THREE.Vector3() as _THREE.Vector3);
  ...
  protected _truckInternal = (
    ...
    if (isPerspectiveCamera(this._camera)) {
      ...
      const truckX =
        (this.truckSpeed * deltaX * targetDistance) / this._elementRect.height;
      const pedestalY =
        (this.truckSpeed * deltaY * targetDistance) / this._elementRect.height;
